...
#
# Git tags are the source of truth for release versions; `git describe --tags`
# derives a uniquely-identified version for each pre-release build. The app's
# Config/Version.xcconfig should hold a frozen placeholder (e.g. 0.0.0-dev)
# for local Xcode debug builds; CI overrides via `xcodebuild MARKETING_VERSION=...`.
#
# Usage:
#   version-derive.sh release <tag-ref>     -> "1.0.2"
#   version-derive.sh beta                  -> "1.0.2-3-gabcdef"
...
#
# Caller MUST use actions/checkout with `fetch-depth: 0`, otherwise
# `git describe` fails for lack of tags.

set -euo pipefail

usage() {
  echo "Usage: $0 <release <tag-ref> | beta | pr>" >&2
  exit 1
}
...
case "$MODE" in
  ...
  beta|pr)
    ...
    # --long forces the "-N-gSHA" suffix even when HEAD is exactly on a
    # tag, so callers always see "1.0.2-0-gabcdef" rather than "1.0.2".
