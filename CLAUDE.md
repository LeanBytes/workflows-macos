...
- **Per-product provisioning profiles + Developer ID cert.** Keychain/ASC/Sparkle keys are team-shared; a product names its profile secret via `devid-profile-secret`/`store-profile-secret` **and** (v0.4.3) its Developer ID certificate via `devid-cert-secret`/`devid-cert-password-secret`, and the matrix leg selects each (`secrets[matrix.product.<key>]`), falling back to the shared `PROV_PROF_*_BASE64` and `DEVELOPER_ID_P12_BASE64`/`DEVELOPER_ID_PASSWORD` — e.g. a product signing under a legacy team kept after an account transfer.
...
- **Signing**: Developer ID p12 imported into an ephemeral `build.keychain`; provisioning profile name is read from the decoded `.provisionprofile` via `security cms -D | plutil -extract Name raw -`. Every `create-keychain` is immediately followed by `security set-keychain-settings` (no `-t`/`-l`) + `unlock-keychain` — a fresh keychain inherits a 300s idle-lock that otherwise fires mid-archive and hangs `codesign` on an unlock prompt no headless runner answers (v0.5.1); `tests/run.sh` asserts the pair at both signing sites.
...
- **Runner**: **`[self-hosted, macOS]` everywhere by default** (v0.4.7). Every job runs self-hosted — including the coordination jobs that used to be `ubuntu-latest` (`prepare`/`discover` in the orchestrators, the per-commit `gate` in `_test`/`memory-watch`), which now run on the self-hosted Mac too. This repo's own `selftest.yml` is the **exception** and stays on `ubuntu-latest`: no self-hosted runner is attached to `workflows-macos` (the org's Mac serves the app repos), so a self-hosted job here queues forever and reports nothing — it silently did not run for 27 days after a v0.4.7-era sweep moved it. Lint + the offline suite need no Mac. Each macOS job's runner is a JSON-encoded input — pass e.g. `'"macos-26"'` to send a job back to a GitHub-hosted runner. Since v0.5.0 the build runner is split per channel: `runs-on-build-direct` (Developer ID archive), `runs-on-build-store` (App Store archive), `runs-on-test`, `runs-on-publish`, plus `runs-on-build` in `distribute-pr`, which has no channels. `runs-on-build-store` exists because altool rejects a bundle built with a beta Xcode (error 90301) while notarization accepts it — so the store archive can need a hosted runner while the direct archive stays self-hosted. **An app with no self-hosted macOS runner MUST override these back to hosted, or its jobs queue forever.**
...
- **Artifact retention**: every uploading workflow takes an `artifact-retention-days` input (default **7**, `type: number`) wired to `retention-days` on `actions/upload-artifact` (build DMG/ZIP/`.pkg`, test-results, memory samples). Build artifacts are consumed within the run (publish downloads them minutes later), so 7 days is generous; an app overrides via the input and the orchestrators pass it through to their callees. (v0.4.6 — before, the build callees hardcoded `7` and `_test`/`memory-watch` defaulted to `90`.)
...
- **When bumping the shared workflow's tag, also bump the `@v<tag>` callouts inside the shared workflows AND in `examples/per-app/`.** This is the convention enforced by every release commit message ("Cross-callouts and example shells bumped to @v0.5.1").
