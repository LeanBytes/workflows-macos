...
- **When bumping the shared workflow's tag, also bump the `@v<tag>` callouts inside the shared workflows AND in `examples/per-app/`.** This is the convention enforced by every release commit message ("Cross-callouts and example shells bumped to @v0.4.7").
