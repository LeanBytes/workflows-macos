...
#   VERSION           sparkle:version (build number, snapshot) OR
...
#
# CHANGELOG.md is no longer consulted (LB-326 dropped that fallback in
# favor of CHANGELOG.json driven by generate-changelog.sh; see
# .github/scripts/generate-changelog.sh). The CI extracts the bullet
# list from CHANGELOG.json's latest version and passes it via NOTES.
...
# the marketing version; snapshot/beta builds pass the build number
# since their marketing version repeats across PR merges.
target = None
