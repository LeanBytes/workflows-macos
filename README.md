...
Pin the `uses:` line to a tag (`@v0.3.30`), not `@main`. Uncomment per-app inputs as needed.
...
jobs:
  pr:
    uses: LeanBytes/workflows-macos/.github/workflows/distribute-pr.yml@v0.3.30
    secrets: inherit
    with:
      # …
```
